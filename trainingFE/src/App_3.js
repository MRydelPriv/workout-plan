...
const App = () => {
    const [trainingss, setTrainigs] = useState([]);
    ...
    useEffect(() => {
        trainingsService
            .getAll()
            .then(initialPersons => {
                console.log('trainingss ', initialPersons);
    ...
    return (
        <div>
            {console.log('sdfghgf   ', trainingss)}
            {historyToDisplay ? historyToDisplay.map(entry => <p> {entry.date} </p>) : null}
            {trainingss.map(training => <button value={trainingss.indexOf(training)} onClick={handleClick}> Training {trainingss.indexOf(training) + 1} </button>)}
            ...
            {timer
                ...
                : null}
            {
                trainingss[currentTraining].map(exercise => <div key={exercise.name + 'h'}>
